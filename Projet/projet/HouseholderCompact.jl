...
function QRHouseholder(A)
    "La fonction suivante calcule et stocke les matrices liées à  la factorisation Qr de Householder
...
# Calcule le produit A'x et le stock dans x.
function mult_Q_transpose_x!(A,x)
    m, n = size(A)
    # Different si c'est une matrice carre.
    if m==n
        for j = 1:(n-1)
            uj = [1 ; A[j+1:end,j]]
            δj = uj'uj
...
# Calcule le produit Ax et le stock dans x.
